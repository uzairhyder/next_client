            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h5>Parent Category List </h5>
                        <div class=""><a class="btn btn-gradient" data-bs-original-title="" title=""
                                href="{{ route('parent-category.create') }}">Add</a></div>
                                    ...
                                    <tbody>
                                        @foreach ($parent_categories as $value)
                                            <tr role="row" class="odd">
                                                ...
                                                <td>
                                                    {{ $value->parent_category_name ?? null }}
                                                </td>
                                                <td>
                                                    <a href="{{ route('parent-category-status', $value->id) }}">
                                                        @if ($value->status == 1)
                                                ...
                                                <td>
                                                    <a href="javascript:void(0);" class="deleteparentcategory"
                                                        data-id="{{ $value->id }}">
                                                        <button class="btn btn-danger btn-xs" type="submit"
                                                            data-original-title="btn btn-danger btn-xs">Delete</button></a>

                                                    <a href="{{ route('parent-category.edit', $value->id) }}"> <button
        ...
        <script>
            $(document).ready(function() {
                ...
                $(".deleteparentcategory").on("click", function() {
                    ...
                    $.ajax({
                        url: "{{route('parent-category.destroy',"id")}}",
                        data: {
                            "id": id,
                            "_token": "{{ csrf_token() }}"
                        },
                        ...
                        success: function(result) {
                            toastr.success('Parent Category Deleted Sucessfully');
                            setTimeout(() => {
                                location.reload();
                            }, 1000);
                        }
