                ...
                <div class="card">
                    <div class="card-header">
                        {{-- <div class="col-xl-12 p-0 left-btn"><a class="btn btn-gradient" data-bs-original-title="" title=""
                                href="{{ route('configuration.create') }}">Create</a></div> --}}
                                    ...
                                    <tbody>
                                        @foreach ($configurations as $value)
                                            <tr role="row" class="odd">
                                                ...
                                                <td>
                                                    {{-- <a href="javascript:void(0);" class="deleteconfiguration"
                                                    data-id="{{ $value->id }}">
                                                    <button class="btn btn-danger btn-xs" type="submit"
                                                        data-original-title="btn btn-danger btn-xs">Delete</button></a> --}}

                                                    <a href="{{ route('configuration.edit', $value->id) }}"> <button
        ...
        <script>
            $(document).ready(function() {
                ...
                $(".deleteconfiguration").on("click", function() {
                    ...
                    $.ajax({
                        url: "{{route('configuration.destroy',"id")}}",
                        data: {
                            "id": id,
                            // '_method': 'DELETE',
                            "_token": "{{ csrf_token() }}"
